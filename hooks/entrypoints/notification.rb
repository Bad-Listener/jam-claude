...
# JAM Claude - Notification entrypoint
...
begin
  ...
  handler = JamClaudeNotificationHandler.new(input_data)
  handler.call
  handler.output_and_exit
rescue JSON::ParserError => e
  warn "[JAM Claude Notification] JSON parsing error: #{e.message}"
  exit 1
rescue StandardError => e
  warn "[JAM Claude Notification] Error: #{e.message}"
