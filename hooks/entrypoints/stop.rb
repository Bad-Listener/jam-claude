...
begin
  ...
  handler = JamClaudeStopHandler.new(input_data)
  handler.call
  handler.output_and_exit
rescue JSON::ParserError => e
  warn "[JAM Claude Stop] JSON parsing error: #{e.message}"
  exit 1
rescue StandardError => e
  warn "[JAM Claude Stop] Error: #{e.message}"
